...
// The number is read directly from tenants.contact_number. A global lock is
// used because the page can load this module more than once through different
// cached/import paths; without a shared lock, two observers can race and each
// append a WhatsApp column after the other has already added one.
...
async function syncTenantListWhatsAppColumn() {
  if (tenantListWhatsAppBusy || window.__biznexcoTenantWhatsAppSyncRunning) return;
  const tbody = document.getElementById('tRows');
  const table = tbody?.closest('table');
  if (!tbody || !table?.tHead?.rows?.[0]) return;

  tenantListWhatsAppBusy = true;
  window.__biznexcoTenantWhatsAppSyncRunning = true;
  ...
  try {
    ...
    // Remove again after the async Supabase read in case another page script
    // inserted a stale WhatsApp column while the query was running.
    Array.from(freshHeader.cells)
      .map((cell, index) => ({ index, text: (cell.textContent || '').trim().toLowerCase() }))
      .filter(x => x.text === 'whatsapp')
      .map(x => x.index)
      .sort((a, b) => b - a)
    ...
    const finalHeader = table.tHead.rows[0];
    const finalLoginIndex = Array.from(finalHeader.cells)
      .findIndex(cell => (cell.textContent || '').trim().toLowerCase() === 'login');
    if (finalLoginIndex < 0) return;

    const whHeader = document.createElement('th');
    whHeader.textContent = 'WhatsApp';
    finalHeader.insertBefore(whHeader, finalHeader.cells[finalLoginIndex + 1] || null);

    Array.from(tbody.rows).forEach(row => {
      const cells = Array.from(row.cells);
      const username = (cells[finalLoginIndex]?.textContent || '').trim().toLowerCase();
      const number = numbersByUsername.get(username) || '';
      const cell = document.createElement('td');
      cell.textContent = number || 'Not saved';
      row.insertBefore(cell, row.cells[finalLoginIndex + 1] || null);
    });
  } catch (error) {
    console.warn('Could not sync tenant WhatsApp column:', error);
  } finally {
    tenantListWhatsAppBusy = false;
    window.__biznexcoTenantWhatsAppSyncRunning = false;
    if (observer) {
